...
% intro
Although, the data collection step gathered data from three systems and stored
them into a single source of truth, it could not have been used to derive any
meaningful information on the correlation between technical debt and work
effort. Therefore, a ``processing stage'' was implemented in the tool, that
allows the calculation of these metrics and visually displays statistics on
using charts.

The processing stage consists of the following steps:
\begin{itemize}
  \item Calculation of work effort per work item;
  \item Calculation of technical debt per work item.
\end{itemize}

The methods of calculation use the collected data of tickets (work report),
commits (``actual'' work) and bug patterns (technical debt). Additional
simplifying assumptions must be taken into consideration as there are many
factors that might realistically influence the work put in by a team member.
...
the work involved and have not been considered in the calculation. This was due
to multiple causes:
...
We assumed that the majority of open-source projects are generally formed
of international team members working remotely
...
The visual analysis of the results was achieved by using a dashboard component
that was requesting data from a RESTful API server. The server retrieved data
from MongoDB, performed calculation and statistical processing steps before
retrieving the results to the dashboard.
