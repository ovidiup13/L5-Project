...
% conclusion
Although only \emph{N} projects have been selected out of the initial 11, using
our data collection process we have managed to retrieve and analyse \emph{M}
commits and \emph{X} issues along with an average of \emph{T} unique quality
issues. Section \ref{results} dives into the results of our analysis. 
...
\begin{enumerate}
  ...
  \item \emph{Search and retrieve ticket(s)}. The current commit message is analysed for mentions
  of ticket IDs. For any ticket ID found, the tool retrieves the ticket from the
...
Data is stored at the end of each substep to avoid an inconsistent commit
stream in case one of the next one fails or the tool crashes.
