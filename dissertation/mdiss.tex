...
completing a feature. By investigating these research questions, this study
contributes:
\begin{itemize}
...
where responding to change is critical \footnote{http://agilemanifesto.org/}.
...
\begin{itemize}
  ...
  \footnote{https://github.com/} and BitBucket \footnote{https://bitbucket.org/}
  also provide an issue tracking tool with each code repository, although they
  do not provide such extensive features as Jira.
  ...
  Spotbugs \footnote{https://spotbugs.github.io/} (formerly FindBugs), CAST,
  Sonargraph, etc.
...
Git is a popular version control tool that tracks changes of the source code
using branches and commits. Projects are stored in a ``repository'' which
contains at least one development line, called a ``branch''. The main
development branch is commonly named the \emph{master} branch. Branches contain
a stream of small units of work, called ``commits'', which are the snapshots of
the source code at a particular point in time. Additionally, each commit
contains relevant metadata with the following fields:
...
\emph{Avgeriou2016}. There are many static analysis tools, many of which have been
reviewed \cite{Fontana2011}. 
...
development environment \cite{Li2015}. For example, static analysis tools
...
The output of a code analysis by SpotBugs contains the following fields:
\begin{itemize}
...
addressed and is described in section \ref{data-collection}. The candidate list
for this project is: 
...
processing phase on a set of software projects. These projects had to satisfy a
...
\begin{itemize}
  ...
  processing consistent. Additionally, this aided our decision in selecting a
  ...
  tool. Multiple build systems could be added if necessary.
\end{itemize}
...
any changes to the initial data model. 

The tool has been built using Java and open-source technologies, and is
available to use by anyone (MIT License
\footnote{https://opensource.org/licenses/MIT}). The tool uses the following
technologies for collecting data:

\begin{itemize}
  \item jGit \footnote{https://www.eclipse.org/jgit/} - is a Java library for interacting with the Git
  ...
  \item GitHub API for Java \footnote{http://github-api.kohsuke.org/} - is a Java library that provides
  ...
  \item Spotbugs - tool for identifying bug patterns in source
  code;
  \item Other technologies - Spring Boot \footnote{https://projects.spring.io/spring-boot/}, 
  Angular \footnote{https://angular.io/} and ChartJS \footnote{http://www.chartjs.org/}
...
shortcuts taken during the testing phases. Secondly, the testing task added an
additional overhead to the build process. In reality, testing is a fundamental
...
mentioned type of technical debt in a mapping study by Li et. al \emph{Li2015},
...
As presented in figure \emph{FIGURE}, a simple method of calculation is to
...
she would get started on the next. Figure \emph{FIGURE} visualises the case.
...
the present is called the \emph{principal} while the extra cost of development
when keeping hold of items is called \emph{interest}. Unfortunately, there is
...
purpose, as all take into consideration various variables for metric
computation. 
